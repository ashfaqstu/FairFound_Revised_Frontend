...
import { Star, MessageCircle, CheckCircle, Lock, UserPlus, X, UserCheck, XCircle, ArrowRight, ThumbsUp, ChevronDown, ChevronUp } from 'lucide-react';
...
interface MentorsProps {
    ...
    onViewMentor?: () => void;
    onPaymentRequired?: (mentor: Mentor) => void; // Redirect to payment
}

// Mock reviews data for each mentor
const MOCK_REVIEWS: Record<string, MentorReview[]> = {
  '1': [
    { id: 'r1', mentorId: '1', reviewerId: 'u1', reviewerName: 'Alex Rivera', reviewerAvatar: 'bg-indigo-500', rating: 5, comment: 'Sarah is an incredible mentor! Her guidance on React architecture helped me land a senior role. Highly recommend!', date: '2024-11-15', helpful: 12 },
    { id: 'r2', mentorId: '1', reviewerId: 'u2', reviewerName: 'Mike Chen', reviewerAvatar: 'bg-emerald-500', rating: 5, comment: 'Very knowledgeable and patient. She breaks down complex concepts into digestible pieces.', date: '2024-10-28', helpful: 8 },
    { id: 'r3', mentorId: '1', reviewerId: 'u3', reviewerName: 'Emily Davis', reviewerAvatar: 'bg-rose-500', rating: 4, comment: 'Great mentor with practical advice. Sessions are always productive.', date: '2024-10-10', helpful: 5 },
  ],
  '2': [
    { id: 'r4', mentorId: '2', reviewerId: 'u4', reviewerName: 'Jordan Lee', reviewerAvatar: 'bg-amber-500', rating: 5, comment: 'Marcus helped me transition from junior to mid-level in just 3 months. His roadmap was spot on!', date: '2024-11-20', helpful: 15 },
    { id: 'r5', mentorId: '2', reviewerId: 'u5', reviewerName: 'Taylor Swift', reviewerAvatar: 'bg-purple-500', rating: 4, comment: 'Very responsive and gives detailed feedback on code reviews.', date: '2024-11-05', helpful: 6 },
  ],
  '3': [
    { id: 'r6', mentorId: '3', reviewerId: 'u6', reviewerName: 'Chris Park', reviewerAvatar: 'bg-cyan-500', rating: 5, comment: 'Elena transformed my portfolio completely. Got 3x more interview callbacks after her guidance.', date: '2024-11-18', helpful: 20 },
    { id: 'r7', mentorId: '3', reviewerId: 'u7', reviewerName: 'Sam Wilson', reviewerAvatar: 'bg-orange-500', rating: 5, comment: 'Best investment in my career. Her design thinking approach is invaluable.', date: '2024-10-25', helpful: 11 },
    { id: 'r8', mentorId: '3', reviewerId: 'u8', reviewerName: 'Lisa Brown', reviewerAvatar: 'bg-pink-500', rating: 4, comment: 'Great mentor for UX/UI designers looking to level up.', date: '2024-09-30', helpful: 7 },
  ],
};

const Mentors: React.FC<MentorsProps> = ({ isSignedUp = true, onSignup, onMentorConnect, connectedMentor, onCancelContract, onViewMentor, onPaymentRequired }) => {
  ...
  const handleViewReviews = (mentor: Mentor) => {
    setReviewsMentor(mentor);
    setShowReviewsModal(true);
  };

  const getMentorReviews = (mentorId: string): MentorReview[] => {
    return MOCK_REVIEWS[mentorId] || [];
  };

  const getAverageRating = (mentorId: string): number => {
  ...
  return (
    <div className="p-8 max-w-6xl mx-auto relative">
      ...
      {/* Current Mentor Card - shown when connected */}
      {connectedMentor && (
        <div className="mb-8">
          <div className="bg-gradient-to-r from-indigo-500 to-purple-600 rounded-2xl p-1">
            <div className="bg-white dark:bg-slate-900 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-xs text-slate-400">Connected since Nov 2024</span>
              </div>
              ...
              <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-4 text-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">Score Improvement</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-6">
            Want to explore other mentors? Cancel your current contract first.
          </p>
        </div>
      )}
      ...
      {/* Mentor Grid - only show when no mentor connected */}
      {!connectedMentor && (
            ...
            <div key={mentor.id} className={`bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all group relative overflow-hidden ${!isSignedUp && index > 1 ? 'blur-sm pointer-events-none' : ''}`}>
                ...
                <div className="flex items-center justify-between py-4 border-t border-b border-slate-50 dark:border-slate-800 mb-4">
                    ...
                    >
                        ...
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{getAverageRating(mentor.id).toFixed(1)}</span>
                        <span className="text-xs text-slate-400 dark:text-slate-500 hover:underline">({getMentorReviews(mentor.id).length} reviews)</span>
                    </button>
                ...
                <div className="flex gap-2 mb-4">
                  <button 
                      onClick={() => handleViewReviews(mentor)}
                      className="flex-1 py-2 rounded-lg font-medium transition-colors flex items-center justify-center gap-1 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 text-sm"
                  >
                ...
                <button 
                    ...
                    className="w-full py-2.5 rounded-lg font-medium transition-colors flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg shadow-indigo-200 dark:shadow-none"
                >
      ...
      {!isSignedUp && !connectedMentor && (
        <div className="mt-8 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-2xl p-8 text-white text-center">
          ...
          <button 
            onClick={onSignup}
            className="bg-white text-indigo-700 px-6 py-3 rounded-xl font-bold hover:bg-indigo-50 transition-colors"
          >
            Sign Up Free
          </button>
        </div>
      ...
      {/* Connection Modal - Payment Required */}
              ...
              <div className="flex items-center gap-4 mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                ...
                <div className="text-center">
                  <div className="flex items-center gap-1 justify-center">
                    ...
                    <span className="text-2xl font-bold text-slate-900 dark:text-white">{getAverageRating(reviewsMentor.id).toFixed(1)}</span>
                  </div>
              ...
              <div className="mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                ...
                <div className="space-y-2">
                  {[5, 4, 3, 2, 1].map(stars => {
                    ...
                    const percentage = getMentorReviews(reviewsMentor.id).length > 0 
                      ? (count / getMentorReviews(reviewsMentor.id).length) * 100 
                      : 0;
                    return (
            ...
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
              <button 
                onClick={() => {
                  ...
                  handleConnectClick(reviewsMentor);
                }}
